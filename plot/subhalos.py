...
from visnap.general import translate_filename
#import pdb
...
def subhalo_functions(halo_objects, Rmass=-1, vcut_factor=0.05,
    ...
    Input
     ...
     legendname - A list strings with the name to give to the legends. If
...
def zoom_z0_subhalo_functions(irate_files, Rmass=-1, halolist=None, vcut_factor=0.05,
                              rcut_factor=1.0, figname='subhalo_function',
    ...
    Input: 
     ...
     rcut_factor - Only subhalos within rcut_factor*host_Rvir will be considered
    ...
    for fcount,irate_file in enumerate(irate_files):
        ...
        arg_sub = argwhere((R_sub < rcut_factor*Rvir_host) & (R_sub > 0) &
                           (Vmax_sub > vcut_factor*Vmax_host))[:,0]
