...
parser.add_option("-N", "--num_snapshots", dest="Nsnaps",
                  default="295", 
                  help="Total number of snapshots (defaults to 295, which is how many snapshots there is in the 650 Mpc/h Box run)")

(options, args) = parser.parse_args()
pair_list_file = options.pair_list_file
trees_path = options.trees_path
snap = int(options.snap)
Nsnaps = int(options.Nsnaps)
...
tree_name1, depth_first_ID1 = find_tree(id1[1], snap, Nsnaps, trees_path)
halo_tprops1 = halo_track(tree_name1, depth_first_ID1, trees_path)

#Get halo2 properties over time
tree_name2, depth_first_ID2 = find_tree(id1[1], snap, Nsnaps, trees_path)
halo_tprops2 = halo_track(tree_name2, depth_first_ID2, trees_path)
