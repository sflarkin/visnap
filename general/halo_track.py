...
def find_tree(orig_halo_id, orig_snapshot, Nsnaps, trees_path,
    ...
    Input:
     ...
     Nsnaps - Total number of snapshots for this run. Used to figure out how
              many snapshots the rockstar tree generator skiped due to not
              enough halos.
    ...
    if not os.path.exists(trees_path+trees_file_base+'.hdf5'):
        print "No IRATE file containing trees found, now generating" 
    ...
    def find_halo_intree(orig_halo_id,orig_snapshot,Nsnaps,in_que,out_que):
        '''
        ...
        for keys in iter(in_que.get,'STOP'):
            for key in keys:
                tree = trees[key]
                offset = Nsnaps-tree['Snap_num'][...].max()
    ...
    for i in range(ncores):
        job = Process(target = find_halo_intree, args = (orig_halo_id,orig_snapshot,Nsnaps,in_que,out_que))
        job.start()
        jobs.append(job)
...
def halo_track(tree_name, halo_depth_first_ID, trees_path,
    ...
    Output:
     halo_props - A dictionary containing halo properties over time,
