#! /usr/bin/python
'''Scriptable module with tools to slice gadget snapshot files'''
...
def slice_snap(input_file, snapshot_number, output_file, x=0, y=0, z=0, dx=1, dy=None,
               dz=None, data_name='Dark_Halo_Region'):
    '''
    Select all dark/halo particles in the IRATE input_file that are enclosed by a region defined
    either by a sphere or a cuboid. Rewrite the selected data in the IRATE output_file under
    output_file/snapshot/data_name.

    Input

     input_file - name of IRATE file to read

     snapshot_number - snapshot to read

     output_file - name of IRATE file to write the selected data (can be the same as the input_file)

     x, y, z - the center of the spehere or the lowest left corner of the cuboid (i.e. xmin, ymin, zmin)

     dx, dy, dz - if dy=None and dz=None, then dx is the radius of the sphere. Otherwise a cuboid is used
                  with dimension dx, dy and dz

     data_name = The name given to the new written data set             

    '''
    
    #open input irate file and read headers

    inirate = h5py.File(input_file,'r')
    insimprops = inirate['SimulationProperties']
    incosmo = inirate['Cosmology']

     try:
         snapnum = int(snapshot_number)
         snapnumS = str(snapshot_number).zfill(5)
     except ValueError:
...
if __name__=="__main__":
    ...
    if sphere:
        slice_snap(inname, snapnum, outname, xcenter, ycenter, zcenter, dx=R, data_name=dname);
    else:
        slice_snap(inname, snapnum, outname, xmin, ymin, zmin, dx, dy, dz, data_name=dname);
